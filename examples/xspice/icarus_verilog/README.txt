...
The simulations require additional files, ivlng.so (or ivlng.DLL)
and ivlng.vpi: they are expected to be in the usual installation location.
To use the versions in a built source tree the .model definitions in
the circuit files must be changed to the very ugly:

.model vlog_ff d_cosim sim_args=["555"]
+ simulation = "../../../release/src/xspice/verilog/.libs/x_ivlng"
+ lib_args = [ "libvvp"
+ "../../../release/src/xspice/verilog/.libs/x_ivlngvpi.so" ]
