...
SUMMARY
    ...
    This function implements the s_cond code model.
...
void cm_s_cond(ARGS)  /* structure holding parms, inputs, outputs, etc.     */
{
    ...
    if (INIT==1) {  /* First pass...allocate storage for previous values... */
        char *num_size_error=
            "\n***WARNING***\nS_XFER: Numerator coefficient array size "
            "is greater than denominator size:\n"
            "        transient analysis will fail.\n";
...
/*  We have to allocate memory and use cm_analog_alloc, because the ITP variables
    ...
    if (ANALYSIS != MIF_AC) {
        if (ANALYSIS == MIF_DC) {
             ...
             * denom coeff to avoid division by zero.
             */
        ...
        *out = 0.0;
        for (i=0; i<num_size; i++) {
            *out = *out + ( *(integrator[i]) * *(num_coefficient[i]) );
        }
